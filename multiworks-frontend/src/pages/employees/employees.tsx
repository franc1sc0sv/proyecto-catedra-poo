    ...
    const fetchEmployees = async () => {
        try {
            ...
            if (axios.isAxiosError(error)) {
                message.error(`Error al cargar empleados: ${error.response?.data?.message || error.message}`);
                ...
                <Card>
                    {loading ? (
                        ...
                        {employee.name} {getStatusTag(employee.status)}
                                        ...
                                        description={
                                            <>
                                                <Text strong>Documento:</Text> {employee.document} <br />
                                                <Text strong>Tipo de Persona:</Text> {employee.person_type} <br />
                                                <Text strong>Tipo de Contrato:</Text> {getContractType(employee.contract_type)} <br />
                                                {employee.email && (
                                                    <>
                                                ...
                                                {employee.phone && (
                                                    <>
                                                        <Text strong>Teléfono:</Text> {employee.phone} <br />
                                                    </>
                                                )}
                                                {employee.address && (
                                                    <>
